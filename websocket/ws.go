...
func GetOKExMarket(wg sync.WaitGroup) {
	...
	//获取公共指数行情
	message := []byte("{\"op\": \"subscribe\", \"args\": [\"index/ticker:BTC-USD\"]}")
	send(message, ws)


	//获取指数的K线数据
	message = []byte("{\"op\": \"subscribe\", \"args\": [\"index/candle60s:BTC-USD\"]}")
	send(message, ws)
	...
	for {
		...
		messageChan <- unzipmsg

		fmt.Println("messageChan size:",len(messageChan))
...
func GetHuobiMarket(wg sync.WaitGroup) {
	ws, err := websocket.Dial(config.HUOBI_WS_URL, "", origin)
	if err != nil {
		log.Fatal(err)
	}
